...
export function useSettings(): UseSettingsReturn {
  ...
  const fetchSettings = async () => {
    try {
      ...
      localStorage.setItem(
        "timerInterval",
        data.default_timer_interval.toString(),
      );
    } catch (err) {
      ...
      const fallbackSettings: UserSettings = {
        ...
        default_timer_interval: parseInt(
  ...
  const updateSettings = async (newSettings: Partial<UserSettings>) => {
    ...
    try {
      ...
      localStorage.setItem(
        "timerInterval",
        updatedSettings.default_timer_interval.toString(),
      );
    } catch (err) {
      setError(
  ...
  const resetSettings = async () => {
    try {
      ...
      localStorage.setItem(
        "timerInterval",
        defaultSettings.default_timer_interval.toString(),
      );
    } catch (err) {
